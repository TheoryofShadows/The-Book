...
What the manifest claims is checked rather than assumed — the `installable`
suite asks the browser to resolve the link at each of the three depths the
generated pages sit at, fetches every icon and reads its real pixel size out
of the PNG header, and asks Chrome itself whether its parser raised any
error. What no check here can do is install anything: there is no Add to Home
Screen in a headless browser and no iOS in this repository at all, so the
install itself is a thing to try on a phone and is claimed nowhere.

**It reads with the network off, and it is never stale to get there.**
`docs/sw.js` is a service worker, generated by `tools/build_sw.py`, and it is
network-first with nothing else in it. Online, every request goes to the
network and the cache is written from the answer; the cache is read only when
a request has actually failed. There is deliberately no timeout that gives up
on a slow network and serves the copy instead, because "current unless the
connection is poor" is not current, and this is a volume that publishes an
accuracy audit. Whatever the worker costs, it cannot cost freshness.

Offline you get the last thing really fetched, and the page says so rather
than looking identical — a cache that answers silently is a claim that what
is on screen is live. The shell is precached so the reader opens at all; the
data is kept as it is read, capped at sixty files, which for works with a
median of 16 KB is a megabyte or so of the chapters somebody actually opened
rather than the twelve-megabyte library nobody asked for. The single-file
copy is excluded outright: one navigation to it would cost more storage than
every chapter a reader will ever open.

The worker is stamped with a hash of everything it serves, which is what
makes a deploy reach a phone. A browser installs a new worker only when its
bytes change, and the caches outlive it, so the stamp is the difference
between a corrected parse arriving and a deleted file being served from
somebody's cache for good. It is a hash of content and not of the clock, so
rebuilding with nothing changed writes the same file.

None of that is taken on trust. `tests/python/test_sw.py` proves the stamp
moves when the data moves, moves when the reader moves, and comes back when
the change is undone — which is what separates a content hash from a
timestamp — and reads the built worker for the shapes that trade freshness
for speed, refusing `stale-while-revalidate` and friends by name. The
`caching` suite then runs one in a browser: it serves a copy of the site from
a temporary directory, changes a file underneath the running page and checks
the page sees the new one, pulls the network and checks it sees the last real
answer, and rebuilds the worker to check the old caches are dropped.
