...
**A bound spine and four leaves, each starting later than the one above it and running its own length.** A codex seen on its fore-edge, and the library on one axis — which in this volume are the same picture. The mark is five rounded rectangles generated by `tools/build_logo.py`, so the tab icon, the header and this are one set of numbers rather than three drawings.
